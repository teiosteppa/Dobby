...
log(`[Hachimi] attach_response = ${attachResponse}`);
...
while (invalidBreakpoints < 10) {
	totalBreakpoints++;
	log(`[Hachimi] Handling breakpoint ${totalBreakpoints} (valid: ${validBreakpoints})`);
	
	let brkResponse = send_command(`c`);
	log(`[Hachimi] brkResponse = ${brkResponse}`);
	...
	if (!tid || !pc || !x0) {
		log(`[Hachimi] Failed to extract registers: tid=${tid}, pc=${pc}, x0=${x0}, x1=${x1}`);
		invalidBreakpoints++;
		continue;
	}
	...
	log(`[Hachimi] tid = ${tid}, pc = ${pcNum.toString(16)}, x0 = ${x0Num.toString(16)}, x1 = ${x1Num.toString(16)}`);
	
	let instructionResponse = send_command(`m${pcNum.toString(16)},4`);
	log(`[Hachimi] instruction at pc: ${instructionResponse}`);
	let instrU32 = littleEndianHexToU32(instructionResponse);
	let brkImmediate = extractBrkImmediate(instrU32);
	log(`[Hachimi] BRK immediate: 0x${brkImmediate.toString(16)} (${brkImmediate})`);
	
	if (brkImmediate !== 0x69 && brkImmediate !== 0x70 && brkImmediate !== 0x71) {
		log(`[Hachimi] Skipping: BRK immediate not 0x69 or 0x70 or 0x71 (was 0x${brkImmediate.toString(16)})`);
		invalidBreakpoints++;
		continue;
	}
	...
	if (brkImmediate === 0x69) { // usual jit mapping
		log(`[Hachimi] Received command to process JIT mapping (0x69)`);
		
		let jitPageAddress = x0Num;
		let size = x1Num > 0n ? x1Num : 0x10000n; // allocate 64 KB if x1 somehow is 0
		log(`[Hachimi] Got RX page address: 0x${jitPageAddress.toString(16)}, preparing region with 0x${size.toString(16)} bytes!`);
		...
		log(`[Hachimi] Patching: dest=0x${destAddr.toString(16)}, src=0x${srcAddr.toString(16)}, size=0x${size.toString(16)}`);
		
		// Unsure exactly why, but anything over 4 MB freezes the app for some reason, so we will set a soft limit
		if (size > 0x400000n) {
			log(`[Hachimi] Size too large (0x${size.toString(16)}), skipping`);
			let pcPlus4 = numberToLittleEndianHexString(pcNum + 4n);
			let pcPlus4Response = send_command(`P20=${pcPlus4};thread:${tid};`);
			log(`[Hachimi] pcPlus4Response = ${pcPlus4Response}`);
			validBreakpoints++;
		...
		try {
			...
			for (let i = 0n; i < size; i += CHUNK_SIZE) {
				...
				if (readRes && readRes.length > 0) {
					...
					if (writeResponse !== "OK") {
						log(`[Hachimi] Write failed at offset ${i.toString(16)}`);
						break;
					}
				}
				if (Number(i / CHUNK_SIZE) % 10 === 0) {
					log(`[Hachimi] Progress: 0x${i.toString(16)}/0x${size.toString(16)}`);
				}
			}
			log(`[Hachimi] Memory write completed!`);
		} catch (e) {
            log(`[Hachimi] Memory write failed: ${e}`);
		}
	...
	log(`[Hachimi] Completed breakpoint ${validBreakpoints}`);
}
log(`[Hachimi] Stopping script (Received 0x71 or too many invalid breakpoints)`);
let detachResponse = send_command(`D`);
log(`[Hachimi] detachResponse = ${detachResponse}`);
